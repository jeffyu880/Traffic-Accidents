...
void printSupplementalImagesResults(sf::RenderWindow& window, std::map<string, Button>& buttons, float timediffTree, float timediffHeap, vector<Node*>& treeAccidents, vector<Accident>& maxHeapAccidents) { //prints supplemental images/text like title, text boxes, etc. for when results tab is selected
...
void displayNewBoard(sf::RenderWindow& window, std::map<string, Button>& buttons, sf::Texture* search, sf::Texture* result, std::vector<sf::Texture*> vect, std::map<string, sf::ConvexShape>& states, sf::Text& c, sf::Text& s, RedBlackTree& rbtree, MaxHeap<Accident>& maxheap, bool results, float& timediffTree, float& timediffHeap, vector<Node*>& treeAccidents, vector<Accident>& maxHeapAccidents) { //RedBlackTree& rbtree, MaxHeap<Accident>& maxheap
	...
	if (buttons["searchResults"].isPressed) { //if results tab is selected:
		...
		printSupplementalImagesResults(window, buttons, timediffTree, timediffHeap, treeAccidents, maxHeapAccidents);
		...
		if (results) {
			...
			if (timediffTree == 0.0f) {
				...
				//Filter by city, state, or neither
				if (!c.getString().isEmpty()) {
					rbtree.FindXMostSevereAccidentsInCity(1, c.getString(), treeAccidents);
				}
				else if (!s.getString().isEmpty()) {
					rbtree.FindXMostSevereAccidentsInState(1, s.getString(), treeAccidents);
				}
				else {
			...
			if (timediffHeap == 0.0f) {
				...
				string ci = c.getString();
				if (!ci.empty() || !st.empty()) {
					sizeVect = 1;
				}
				else {
					sizeVect = 10;
				...
				while(maxHeapAccidents.size() != sizeVect) { //get top 10 accidents for heap, filter by city, state, or neither
					...
					if (!ci.empty()) {
						...
						if (heap.top().state.compare(st) == 0) {
							maxHeapAccidents.push_back(heap.top());
							heap.pop();
						}
						else {
