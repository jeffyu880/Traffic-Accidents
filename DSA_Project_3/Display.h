...
void displayNewBoard(sf::RenderWindow& window, std::map<string, sf::ConvexShape>& states, unordered_map<string, RedBlackTree> treeStateMap, unordered_map<string, MaxHeap<Accident>> maxStateMap, vector<StateButton>& stateButtons, string stateSelected, float& timediffTree, float& timediffHeap, vector<Accident>& accidents) {
	...
	if (treeStateMap.find(stateSelected) != treeStateMap.end()) {
		...
		for (int i = 0; i < 6; i++) { //get top 10 accidents for heap
	...
	if (timediffTree == 0.0f) {
		...
		treeStateMap[stateSelected].FindXMostSevereAccidents(6,nodeVector); //get top 10 accidents for tree
	...
	if (timediffHeap == 0.0f) {
		...
		heap.MaxKElements(6, accidentVector);
...
void displayNewBoard(sf::RenderWindow& window, std::map<string, sf::ConvexShape>& states, unordered_map<string, RedBlackTree> treeStateMap, unordered_map<string, MaxHeap<Accident>> maxStateMap, vector<StateButton>& stateButtons, vector<Accident>& accidents, float& timediffTree, float& timediffHeap) {
